...
class LinearRegression():
	...
	def fit_dg(self,x,y):
		...
		self.fit_NormalEquation()


	def fit_NormalEquation(self):
		self.theta2 = np.dot(np.dot(np.linalg.inv(np.dot(self.x.T,self.x)),self.x.T),self.y)
		self.CostNormalEquation = self.funcionDeCoste(self.theta2)
	...
	def predict(self,x):
		...
		for j in range(self.n-1):
			...
			axs[fil,col].plot(self.x[:,j+1],self.y,'cx')
			axs[fil,col].plot(d,self.theta[0]+d*self.theta[j+1],'r-',label='DesensoGradiente')
			axs[fil,col].plot(x[j+1],self.theta[0]+x[j+1]*self.theta[j+1],'bo',linewidth=2, markersize=6)
			axs[fil,col].plot(d,self.theta2[0]+d*self.theta2[j+1],'g-',label='NormalEquation')
			axs[fil,col].plot(x[j+1],self.theta2[0]+x[j+1]*self.theta2[j+1],'yo',linewidth=2, markersize=6)
			axs[fil,col].legend()
			col += 1 
			if col==3 or col==6:
				fil += 1
	...
	def graficaCosteInteraciones(self):
		plt.plot(np.arange(0,self.it),self.J_History,label='DesensoGradiente')
		plt.plot(1,self.CostNormalEquation,'ro',label='NormalEquation')
		plt.legend()
